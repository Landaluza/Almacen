Public Class DataGridViewPrintDocument    
    ...
    Protected Overrides Sub OnPrintPage(ByVal ev As Printing.PrintPageEventArgs)
        ...
        Dim leftMargin As Double = ev.MarginBounds.Left
        ...
        ev.Graphics.DrawString("Página:" & nPagina.ToString, Me.DataFont, Brushes.Black, Convert.ToInt32(leftMargin), Convert.ToInt32(yPos), New StringFormat())
        count += 1
        'imprimir fecha de impresion
        yPos = topMargin + (count * Me.DataFont.GetHeight(ev.Graphics))
        ev.Graphics.DrawString("Fecha:" & Me.FechaReporte, Me.DataFont, Brushes.Black, Convert.ToInt32(leftMargin), Convert.ToInt32(yPos), New StringFormat())
        count += 1
        'imprimir hora de impresión
        yPos = topMargin + (count * Me.DataFont.GetHeight(ev.Graphics))
        ev.Graphics.DrawString("Hora:" & Me.HoraReporte, Me.DataFont, Brushes.Black, Convert.ToInt32(leftMargin), Convert.ToInt32(yPos), New StringFormat())
        count += 1
        ...
        For i As Integer = 0 To Me.Encabezado.Length - 1
            ...
            ev.Graphics.DrawString(LTrim(Me.Encabezado(i)) & ControlChars.NewLine, Me.HeaderFont, Brushes.Black, Convert.ToInt32(leftMargin), Convert.ToInt32(yPos), New StringFormat())
            count += 1
        Next
        ...
        yPos = topMargin + (count *
        Me.HeaderFont.GetHeight(ev.Graphics))
        ev.Graphics.DrawLine(Pens.Black, Convert.ToInt32(leftMargin), Convert.ToInt32(yPos), Convert.ToInt32(ev.PageBounds.Width - ev.MarginBounds.Left), Convert.ToInt32(yPos))
        ...
        ev.Graphics.DrawString(line, printFont, Brushes.Black,Convert.ToInt32(leftMargin), Convert.ToInt32(yPos), New StringFormat())
